...
class GrammarTransformer(Transformer):
	...
	def sprite_def(args):
		...
		return {
				...
				"procedures": [
					i
					for i in args
					if i["type"] == "procedures_definition"]}
	costume_list = lambda args: {
			"type": "costume_list",
			"costumes": args}
	ident = lambda args: {
			"type": "ident",
	...
	stmts = lambda args: {
			"type": "stmts",
			"stmts": args}
	block_stmt = lambda args: {
			"type": "block_stmt",
			"body": args[0]}
	proc_def = lambda args: {
			"type": "procedures_definition",
			"name": args[0]["name"],
			"params": args[1]["params"],
			"warp": "false",
			"body": args[2]["body"]["stmts"]}
	proc_def_warp = lambda args: {
			"type": "procedures_definition",
			"name": args[0]["name"],
			"params": args[1]["params"],
			"warp": "true",
			"body": args[2]["body"]["stmts"]}
	param_list = lambda args: {
			"type": "param_list",
			"params": [{
				"type": "param",
				"name": p["name"]}
	...
	arg_list = lambda args: {
			"type": "arg_list",
			"args": args}
	addition = lambda args: {
			"type": "operator_add",
			"NUM1": args[0],
			"NUM2": args[1]}
	subtraction = lambda args: {
			"type": "operator_subtract",
			"NUM1": args[0],
			"NUM2": args[1]}
	multiplication = lambda args: {
	...
	negate = lambda args: {
			...
			"NUM1": 0.0,
			"NUM2": args[0]}
	less_than = lambda args: {
			"type": "operator_lt",
			"OPERAND1": args[0],
			"OPERAND2": args[1]}
	greater_than = lambda args: {
			"type": "operator_gt",
			"OPERAND1": args[0],
			"OPERAND2": args[1]}
	equal_to = lambda args: {
			"type": "operator_equals",
			"OPERAND1": args[0],
			"OPERAND2": args[1]}
	logical_or = lambda args: {
			"type": "operator_or",
			"OPERAND1": args[0],
			"OPERAND2": args[1]}
	logical_and = lambda args: {
			"type": "operator_and",
			"OPERAND1": args[0],
			"OPERAND2": args[1]}
	logical_not = lambda args: {
			"type": "operator_not",
			"OPERAND": args[0]}
	var_decl = lambda args: {
			"type": "var_decl",
			"name": args[0]["name"]}
	arr_decl = lambda args: {
			"type": "arr_decl",
			"name": args[0]["name"]}
	if_stmt = lambda args: {
			"type": "control_if",
	...
	if_else_stmt = lambda args: {
			"type": "control_if_else",
			"CONDITION": args[0],
			"true_branch": args[1]["body"]["stmts"],
			"false_branch": args[2]["body"]["stmts"]}
	if_elif_stmt = lambda args: {
			"type": "control_if_else",
			"CONDITION": args[0],
			"true_branch": args[1]["body"]["stmts"],
			"false_branch": [args[2]]}
	...
	var_deq = lambda args: {
			...
			"value": {
				...
				"NUM1": args[0],
				"NUM2": args[1]}}
	until_loop = lambda args: {
			"type": "control_repeat_until",
			"CONDITION": args[0],
			"body": args[1]["body"]["stmts"]}
	while_loop = lambda args: {
			"type": "control_while",
			"CONDITION": args[0],
			"body": args[1]["body"]["stmts"]}
	repeat_loop = lambda args: {
			"type": "control_repeat",
			"TIMES": args[0],
			"body": args[1]["body"]["stmts"]}
	forever_loop = lambda args: {
